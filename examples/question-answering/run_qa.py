...
class DataTrainingArguments:
    ...
    data_language: Optional[str] = field(
        default=None, metadata={"help": "Language to use in case of TyDiQA."}
    )
...
def filter_by_language(data: DatasetDict, language: str) -> DatasetDict:
    for split in data:
        indices = [i for i, val in enumerate(data[split]['id']) if val.startswith(language)]
        data[split] = data[split].select(indices)
    return data
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.dataset_config_name == 'tydiqa':
            assert data_args.data_language is not None
            if data_args.data_language in ["thai", "japanese"]:
                datasets = load_dataset("khalidalt/tydiqa-goldp", data_args.data_language)
                datasets = datasets.rename_column('document_title', 'title').rename_column('passage_text', 'context').rename_column('question_text', 'question')

                def change_answer(example):
                    example['answers']['answer_start'] = example['answers']['start_byte']
                    del example['answers']['start_byte']
                    del example['answers']['limit_byte']
                    return example
                datasets = datasets.map(change_answer)
            else:
                datasets = filter_by_language(datasets, data_args.data_language)
    else:
        data_files = {}
    ...
    if training_args.do_eval:
        if data_args.eval_split not in datasets:
            raise ValueError(f"--do_eval requires a {data_args.eval_split} dataset")
        eval_examples = datasets[data_args.eval_split]
