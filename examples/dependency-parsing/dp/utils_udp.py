...
class DataCollatorForDependencyParsing:
    ...
    def __call__(self, features):
        ...
        #    print(f'{k}: ' + ' '.join([str(len(x)) for x in v]))
...
class DependencyParsingTrainer(TLRTrainer):
    ...
    def prediction_loop(
        ...
        for step, inputs in enumerate(dataloader):
            if self.lang_adapters:
                ...
                if len(model.active_adapters) > 2:
                    multi_task = model.active_adapters[-2]
                    model.set_active_adapters(ac.Stack(adapter, multi_task, task_adap))
                else:
