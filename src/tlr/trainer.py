...
from transformers.data.data_collator import DataCollator, DataCollatorWithPadding, default_data_collator, DataCollatorForLanguageModeling
...
os.environ["WANDB_PROJECT"] = "tlr-adapters"

logger = logging.get_logger(__name__)


class TLRTrainer(Trainer):
    ...
    def __init__(
        ...
        self.lang_adapters = lang_adapters
        ...
        **kwargs,
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                for _ in train_dataloader:
                    break
        if self.lang_adapters:
            num_lang_adapters = len(self.lang_adapters)
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                adapter = None
                if self.lang_adapters:
                    adapter = self.lang_adapters[step % num_lang_adapters]
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss += self.training_step(model, inputs, adapter)
                else:
                    tr_loss += self.training_step(model, inputs, adapter)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], adapter: str = None) -> torch.Tensor:
        """
        ...
        model.train()
        if adapter:
            task_adap = model.active_adapters[-1]
            model.train_adapter([task_adap])
            model.set_active_adapters(ac.Stack(adapter, task_adap))
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if self.lang_adapters:
                adapter = self.lang_adapters[step % num_lang_adapters]
            # Prediction step
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys, adapter=adapter)
